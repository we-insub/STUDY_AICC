...
links.Timeline.prototype.getAxisDom = function() {
    return this.dom.axis.frame;
}

// 타임라인 일자 세팅
links.Timeline.prototype.setRangeMinMax = function(min, max) {
    this.options.min = min;
    this.options.max = max;
}

// 웹포스 작업자 여부
links.Timeline.prototype.setWebpos = function(arr) {
                ...
                if (Array.isArray(items[i2].member)) {
                    memberfor : for (var m = 0; m < items[i2].member.length; m++) {
                        if (items[i2].member[m].mName == arr[i].memId) {
                            ...
                            timeline.changeItem(i2, items[i2], false);
                            break itemsfor;
                        }
                    }
                }
...
// 웹포스 버튼 노출 여부
links.Timeline.prototype.setWebposButtonDisplay = function(arr) {
    var items = this.items;
    itemsfor : for (var i = 0; i < items.length; i++) {
        items[i].webposButton = false;
        arrfor : for (var i2 = 0; i2 < arr.length; i2++ ) {
            if (arr[i2].schedNo = items[i].schedNo) {
                items[i].webposButton = true;
                break arrfor;
            }
        }
...
links.Timeline.prototype.setData = function(data) {
    ...
    var items = this.items;
    //console.log(this);
    this.deleteGroups(); // 그룹은 고정이기때문에 초기화 하면 안됨
...
links.Timeline.prototype.reflowFrame = function() {
    ...
    var frameHeight = dom.frame ? dom.frame.clientHeight : 0,
        // frameWidth  = dom.frame ? 2000 : 0
        frameWidth  = dom.frame ? dom.frame.offsetWidth : 0
    ;
    //var
    // console.log(dom.frame);
...
links.Timeline.prototype.repaintFrame = function() {
    ...
    if (!dom.frame) {
        ...
        dom.frame.className = "timeline-frame ui-widget ui-widget-content ui-corner-all stt_tquick";
    ...
    if (!dom.content) {
        ...
        // blackLayer.style.opacity = "0.5";
        // blackLayer.style.zIndex = "50";
        // blackLayer.className = "blackLayer";
        // dom.content.appendChild(blackLayer);
        // dom.contentblackLayer = blackLayer;
...
links.Timeline.prototype.repaintAxis = function() {
    ...
    if (!axis.frame) {
        ...
        axis.frame.setAttribute("id", "axisFrame");
...
links.Timeline.prototype.repaintAxisCharacters = function () {
    ...
    if (!axis.characterMinor) {
        ...
        characterMinor.style.borderLeft = "1px solid #eee";
    ...
    if (!axis.characterMajor) {
        ...
        characterMajor.style.borderLeft = "1px solid #eee";
...
links.Timeline.prototype.repaintAxisHorizontal = function() {
    ...
    if (hasAxis) {
        if (!axis.backgroundLine) {
            ...
            backgroundLine.style.zIndex = "1000";
...
links.Timeline.prototype.repaintGroups = function() {
    ...
    if (!frame) {
        frame =  document.createElement("DIV");
        frame.className = "timeline-groups-axis stt_tquick_channel";
    ...
    for (var i = current; i < needed; i++) {
        ...
        // REST 그룹은 별도 처리
        if (group.content == "REST") {
            var html = handlebarsFn("#nonWorkSch", undefined);
            $(html).data("groupNo", group.groupNo);
            label.innerHTML = html;
            frame.appendChild(label);
            labels[i] = label;

        } else {
            var html = handlebarsFn("#WorkSch", {
...
links.Timeline.prototype.repaintCustomButton = function () {
...
links.Timeline.prototype.timeToScreen = function(time) {
    var conversion = this.conversion;
    return (time.valueOf() - conversion.offset)
        *
        // conversion.factor
        0.000185
        ;
...
links.Timeline.prototype.onMouseUp = function (event) {
    ...
    // 아이템선택상태가 아닌 삭제 버튼
    /*if (this.dom.items.deleteButton) {
        if (params.target.className === this.dom.items.deleteButton[0].className) {
            // delete item
            this.selectItem(params.itemIndex);
            this.trigger('select');
 ...
 *                                {Number} height
 */
links.Timeline.Item = function (data, options, parentDom) {
    if (data) {
 ...
 *                                {Number} height
 */
links.Timeline.ItemFloatingRange = function (data, options, parentDom) {
    links.Timeline.Item.call(this, data, options, parentDom);
};
...
links.Timeline.ItemFloatingRange.prototype.select = function () {
    var dom = this.dom;
    links.Timeline.addClassName(dom, 'timeline-event-selected ui-state-active');
    $(dom).find(".stt_box").addClass("on");
    $(dom).find(".stt_box_etc").addClass("on");
};
...
links.Timeline.ItemFloatingRange.prototype.unselect = function () {
    var dom = this.dom;
    links.Timeline.removeClassName(dom, 'timeline-event-selected ui-state-active');
    $(dom).find(".stt_box").removeClass("on");
    $(dom).find(".stt_box_etc").removeClass("on");
};
...
links.Timeline.ItemFloatingRange.prototype.updateDOM = function () {
    var tag;
    var divBox = this.dom;
    var members = (! this.member)?[]:this.member;
    var parentDos = this.parentDom;
    var restTxt;

    // console.log(this);
    if (divBox) {
        // update contents
        var programTime = (this.end.getTime() - this.start.getTime()) / 60000; // 분 단위로 변경.
        if (this.group.content == "REST") {
            // 작업외 일정
            if (this.className) {
                restTxt = (this.className.indexOf("rest") > -1)?"휴식시간":"식사시간";
            }
            var paramObj = {};
            paramObj.content = this.content;
            paramObj.class = this.className;
            paramObj.programTime = programTime;
            paramObj.restTxt = restTxt;
            if (this.member) {
                if (this.member.profImgData) {
                    paramObj.profImgData = this.member.profImgData;
                    paramObj.isProfImgData = true;
                }
            }
            tag = handlebarsFn("#nonWorkBlock", paramObj);
        } else {

            for (var m = 0, mLen = members.length; m < mLen; m++) {
                members[m].isTextBox = true;
                if (members.length >= 4 && members[m].workTime == 5) {
                    members[m].isTextBox = false;
                }
            }
            // 작업 일정
            // var deleteButton = parentDos.items.deleteButton;

            tag = handlebarsFn("#WorkBlock", {
                members:members,
                content:this.content,
                class:this.className,
                programTime:programTime,
                workType:this.workTypeClass,
                schedNo:this.schedNo,
                progNm:this.content,
                schedType:this.schedType,
                webposButton:this.webposButton
            });

            //parentDos.items.deleteButton = $(tag).find(".timeline-navigation-delete");

        }
        divBox.firstChild.innerHTML = tag;
        // divBox.firstChild.innerHTML = this.content;
        // update class
        divBox.className = "timeline-event timeline-event-range ui-widget ui-state-default";
...
links.Timeline.prototype.deleteGroupItems = function (obj) {
    var me = this;
    var items = this.items;
    var indexArr = [];
...
/**
 * 서버 요청을 통해 현제 작업 시작한 구룹별 담당자의 데이터를 가져와서 group데이터에 세팅합니다.
 *
 *
 *
 */
links.Timeline.prototype.setMemberLive = function() {
    ...
    for (var g = 0; g < groups.length; g++) {
        memberArr = groups[g].member;
        if (memberArr) {
            for(var m = 0; m < memberArr.length; m++){
                member = memberArr[m];
                if (liveMemberObj) {
                    for (var g2 = 0; g2 < liveMemberObj.length; g2++) {
                        if (liveMemberObj[g2].mno == member.mno) {
                            // 작업중 체크
                            member.isOn = true;
                        }
                    }
...
links.Timeline.prototype.createItem = function(itemData) {
    ...
    if (type in this.itemTypes) {
        return new this.itemTypes[type](data, {'top': initialTop}, this.dom)
    }

    // 잘못된 타입일 경우 에러, 기본 타입으로 세팅하고 그린다.
...
links.Timeline.prototype.changeItem = function (index, itemData, preventRender) {
    ...
    var newItem = this.createItem({
        ...
        'groupNo':   itemData.hasOwnProperty('groupNo') ?   itemData.groupNo :   this.getGroupName(oldItem.groupNo),
 ...
 *
 *  this.groups = ['A','B'];
 this.groupIndexes = {'A':0,'B':1};
 *
 */
links.Timeline.prototype.getGroup = function (itemData) {
    var groups = this.groups,
    ...
    if (groupIndex == undefined && itemData.groupNo != undefined) { // not null or undefined 새로운 그룹 이름, 그룹 생성
        ...
        if (this.options.groupsOrder == true) {
            /*groups = groups.sort(function (a, b) {
                if (a.content == 'REST') { // REST 그룹은 첫번쨰 행으로 고정
                    return -1;
                }
